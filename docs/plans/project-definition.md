# Project Definition

## a) Interests

- Software Engineering
- Harness Engineering
- Machine Learning

> Since getting my first ever personal computer at the age of 10, I have been deeply interested in the software world. The famous sandbox game Minecraft's customization features exposed myself to the very basic mechanisms such as file extensions, then later on brought me into the world of networking. For more than 6 years, my primary interest and passion have been software engineering. Despite never being a successful programmer, tinkering with different components of software and getting to know the existence of new areas of the software world fascinated me. Combined with my STEM oriented academic success, my future career directionality was set to this path, and I am currently in the process of narrowing down the path to a more specific occupation.

> Late 2024, following the release of MCP - Model Context Protocol - by Anthropic, my interests expanded to artificial intelligence and agentic infrastructure. The release of this new powerful tool for large language models led to the era of agentic artificial intelligence, allowing it to function as a fully capable assistant that has access to broad context and practical tools.

> What this event truly taught me was the importance of harness around raw models. It was a transition from maximizing raw model intelligence, to providing the appropriate environment, set of tools, and ports for certain models to perform at its highest capabilities at given constrained circumstances. To put it simple, instead of squishing in a giant model to get the performance needed, skilled harness engineering allows much smaller models to perform at the same level or complete tasks that are impossible without them in a more limited environment - like a mobile device. I became deeply interested in this area, and have been expanding my knowledge and insight in this area since then.

## b) Initial Draft

### Learning Goal

> To learn how to harness and optimize on-device large language models for practical real-world applications by investigating how model selection, quantization, prompt design, and context management affect response quality and device resource use.

### Product Goal

> To create a functional, privacy-first note-taking application that captures information through written notes, AI-assisted conversations, and voice memos, then uses on-device large language models to organize, connect, and retrieve the stored information.

## c) Panel Feedback

### Raw Feedback

- simplify the language of the learning goal
- backwards planning
- what has to be done by when to ensure the project is successful
- consider the simpler language when applicable

### Processed Feedback
